...
public class MealServiceTest {
  ...
  public void getMeals_ShouldReturnMeals_WhenCalled() {
    ...
    List<Meal> meals =
        List.of(
            Meal.builder().name("Kebab1").prices(kebabPrices).build(),
            Meal.builder().name("Kebab2").prices(kebabPrices).build());
  ...
  public void findMealByName_ShouldReturnMeal_WhenMealExists() {
    ...
    Meal meal = Meal.builder().name("Kebab").prices(kebabPrices).build();
  ...
  public void addMeal_ShouldAddMeal_WhenMealDoesNotExist() {
    ...
    NewMealRequest request =
        NewMealRequest.builder().newMealName("Kebab").ingredients(ingredients).build();

    when(mealRepository.findByName(request.newMealName())).thenReturn(Optional.empty());

    Meal newMeal = Meal.builder().name(request.newMealName()).build();
  ...
  public void addMeal_ShouldThrowMealAlreadyExistsException_WhenMealExists() {
    ...
    NewMealRequest request = NewMealRequest.builder().newMealName("Kebab").build();

    Meal existingMeal = Meal.builder().name("Kebab").build();

    when(mealRepository.findByName(request.newMealName())).thenReturn(Optional.of(existingMeal));

    assertThrows(
        MealAlreadyExistsException.class,
        () -> {
  ...
  public void removeMeal_ShouldRemoveMeal_WhenCalled() {
    ...
    Meal meal = Meal.builder().name("Kebab").prices(new EnumMap<>(Size.class)).build();
