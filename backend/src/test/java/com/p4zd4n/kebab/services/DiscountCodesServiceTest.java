...
public class DiscountCodesServiceTest {
    ...
    public void getDiscountCodes_ShouldReturnDiscountCodes_WhenCalled() {
        ...
        List<DiscountCode> discountCodes = Arrays.asList(
                ...
                new DiscountCode(exampleCode2, BigDecimal.valueOf(30), LocalDate.now().plusMonths(1), 1L)
        );
    ...
    public void getDiscountCode_ShouldThrowDiscountCodeExpiredException_WhenDiscountCodeExpired() {
        ...
        DiscountCode discountCode = new DiscountCode(exampleCode1, BigDecimal.valueOf(20), LocalDate.now().minusMonths(1), 3L);

        when(discountCodesRepository.findByCode(exampleCode1)).thenReturn(Optional.of(discountCode));

        assertThrows(DiscountCodeExpiredException.class, () -> {
            discountCodesService.getDiscountCode(exampleCode1);
        });
    ...
    public void addDiscountCode_ShouldAddDiscountCode_WhenSameDiscountCodeDoesNotExist() {
        ...
        DiscountCode discountCode = new DiscountCode(request.code(), request.discountPercentage(), request.expirationDate(), request.remainingUses());
        ...
        assertEquals("Successfully added new discount code: '" + exampleCode1 + "'", response.message());
    ...
    @Test
    public void addDiscountCode_ShouldThrowDiscountCodeAlreadyExistsException_WhenSameDiscountCodeExists() {
    ...
    @Test
    public void findDiscountCodeByCode_ShouldThrowDiscountCodeNotFoundException_WhenDiscountCodeDoesNotExist() {

        when(discountCodesRepository.findByCode(exampleCode1)).thenThrow(new DiscountCodeNotFoundException(exampleCode1));
