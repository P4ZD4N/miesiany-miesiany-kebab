...
public class ContactControllerTest {
  ...
  public void getContacts_ShouldReturnContacts_WhenCalled() throws Exception {
    ...
    List<ContactResponse> contactResponsesList =
        Arrays.asList(
            ContactResponse.builder().contactType(ContactType.TELEPHONE).value("123456789").build(),
    ...
    mockMvc
        .perform(get("/api/v1/contact/contacts"))
  ...
  public void updateContact_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    UpdatedContactResponse expectedResponse =
        UpdatedContactResponse.builder()
            .statusCode(HttpStatus.OK.value())
            .message(
                "Successfully updated contact of type '" + existingContact.getContactType() + "'")
            .build();

    when(contactService.findContactByType(request.contactType())).thenReturn(existingContact);
    when(contactService.updateContact(any(Contact.class), any(UpdatedContactRequest.class)))
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.message",
                is(
                    "Successfully updated contact of type '"
                        + existingContact.getContactType()
                        + "'")));
  ...
  public void updateContact_ShouldReturnBadRequest_WhenInvalidPhone() throws Exception {
    ...
    when(contactService.findContactByType(request.contactType())).thenReturn(existingContact);
    when(contactService.updateContact(existingContact, request))
        .thenThrow(new InvalidPhoneException(request.newValue()));
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(new ContactController(contactService))
  ...
  public void updateContact_ShouldReturnBadRequest_WhenInvalidEmail() throws Exception {
    ...
    when(contactService.findContactByType(request.contactType())).thenReturn(existingContact);
    when(contactService.updateContact(existingContact, request))
        .thenThrow(new InvalidEmailException(request.newValue()));
  ...
  public void updateContact_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(put("/api/v1/contact/update-contact").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
}
