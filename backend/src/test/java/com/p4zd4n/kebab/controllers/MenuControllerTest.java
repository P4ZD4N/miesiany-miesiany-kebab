...
public class MenuControllerTest {
  ...
  @MockBean private BeverageService beverageService;

  @MockBean private AddonService addonService;
  ...
  public void getBeverages_ShouldReturnBeverages_WhenCalled() throws Exception {
    ...
    List<BeverageResponse> beveragesList =
        Arrays.asList(
    ...
    mockMvc
        .perform(get("/api/v1/menu/beverages"))
  ...
  public void addBeverage_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    NewBeverageResponse response =
        NewBeverageResponse.builder()
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message", is("Successfully added new beverage with name 'Coca-Cola'")));
  }
  ...
  public void addBeverage_ShouldReturnConflict_WhenBeverageAlreadyExists() throws Exception {
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(
                new MenuController(beverageService, addonService, mealService, ingredientService))
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message", is("Beverage with the same name and capacity already exists!")));
  ...
  public void addBeverage_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(post("/api/v1/menu/add-beverage").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void updateBeverage_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    UpdatedBeverageResponse expectedResponse =
        UpdatedBeverageResponse.builder()
            .statusCode(HttpStatus.OK.value())
            .message("Successfully updated beverage with name 'Coca-Cola'")
            .build();

    when(beverageService.findBeverageByNameAndCapacity(
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.status_code", is(HttpStatus.OK.value())))
        .andExpect(
            jsonPath("$.message", is("Successfully updated beverage with name 'Coca-Cola'")));

    verify(beverageService, times(1))
        .findBeverageByNameAndCapacity(
            request.updatedBeverageName(), request.updatedBeverageOldCapacity());
  ...
  public void updateBeverage_ShouldReturnConflict_WhenBeverageAlreadyExists() throws Exception {
    ...
    when(beverageService.findBeverageByNameAndCapacity(
            request.updatedBeverageName(), request.updatedBeverageOldCapacity()))
        .thenReturn(existingBeverage);
    when(beverageService.updateBeverage(existingBeverage, request))
        .thenThrow(new BeverageAlreadyExistsException());
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.status_code", is(409)))
        .andExpect(
            jsonPath("$.message", is("Napoj z taka sama nazwa i pojemnoscia juz istnieje!")));
  ...
  public void updateBeverage_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(put("/api/v1/menu/update-beverage").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void removeBeverage_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    RemovedBeverageRequest request =
        RemovedBeverageRequest.builder()
            .name("Coca-Cola")
            .capacity(BigDecimal.valueOf(0.5))
            .build();

    RemovedBeverageResponse response =
        RemovedBeverageResponse.builder()
            .statusCode(HttpStatus.OK.value())
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.status_code", is(HttpStatus.OK.value())))
        .andExpect(
            jsonPath("$.message", is("Successfully removed beverage with name 'Coca-Cola'")));

    verify(beverageService, times(1))
        .findBeverageByNameAndCapacity(request.name(), request.capacity());
  ...
  public void getAddons_ShouldReturnAddons_WhenCalled() throws Exception {
    ...
    List<AddonResponse> addonList =
        Arrays.asList(
            AddonResponse.builder().name("Jalapeno").price(BigDecimal.valueOf(3)).build(),
            AddonResponse.builder().name("Herbs").price(BigDecimal.valueOf(2)).build());

    when(addonService.getAddons()).thenReturn(addonList);

    mockMvc
        .perform(get("/api/v1/menu/addons"))
        .andExpect(status().isOk())
  ...
  public void addAddon_ShouldReturnConflict_WhenAddonAlreadyExists() throws Exception {
    ...
    when(addonService.addAddon(request))
        .thenThrow(new AddonAlreadyExistsException(request.newAddonName()));
  ...
  public void addAddon_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(post("/api/v1/menu/add-addon").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void updateAddon_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    Addon existingAddon = Addon.builder().name("Jalapeno").price(BigDecimal.valueOf(3)).build();
    ...
    UpdatedAddonResponse expectedResponse =
        UpdatedAddonResponse.builder()
            .statusCode(HttpStatus.OK.value())
            .message("Successfully updated addon with name 'Jalapeno'")
            .build();

    when(addonService.findAddonByName(request.updatedAddonName())).thenReturn(existingAddon);
  ...
  public void updateAddon_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(put("/api/v1/menu/update-addon").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void removeAddon_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    RemovedAddonRequest request = RemovedAddonRequest.builder().name("Jalapeno").build();
  ...
  public void addMeal_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    NewMealRequest request =
        NewMealRequest.builder()
            .newMealName("Kebab")
            .ingredients(ingredientResponses)
            .prices(kebabPrices)
            .build();
  ...
  public void addMeal_ShouldReturnConflict_WhenMealAlreadyExists() throws Exception {
    ...
    when(mealService.addMeal(request))
        .thenThrow(new MealAlreadyExistsException(request.newMealName()));
  ...
  public void addMeal_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(post("/api/v1/menu/add-meal").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void updateMeal_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    Meal existingMeal = Meal.builder().name("Kebab").prices(kebabPrices).build();

    existingMeal.addIngredient(
        Ingredient.builder().name("Mutton").ingredientType(IngredientType.MEAT).build());

    UpdatedMealRequest request =
        UpdatedMealRequest.builder()
            .updatedMealName("Kebab")
            .updatedPrices(kebabPrices)
            .updatedIngredients(updatedIngredients)
            .build();

    UpdatedMealResponse expectedResponse =
  ...
  public void updateMeal_ShouldReturnNotFound_WhenIngredientNotExist() throws Exception {
    ...
    when(mealService.updateMeal(existingMeal, request))
        .thenThrow(new IngredientNotFoundException("Chicken"));
  ...
  public void updateMeal_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(put("/api/v1/menu/update-meal").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void removeMeal_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    Meal existingMeal = Meal.builder().name("Kebab").prices(new EnumMap<>(Size.class)).build();

    existingMeal.addIngredient(
        Ingredient.builder().name("Mutton").ingredientType(IngredientType.MEAT).build());

    RemovedMealRequest request = RemovedMealRequest.builder().name("Kebab").build();
  ...
  public void getIngredients_ShouldReturnIngredients_WhenCalled() throws Exception {
    ...
    List<IngredientResponse> ingredientsList =
        Arrays.asList(
    ...
    mockMvc
        .perform(get("/api/v1/menu/ingredients"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(2)))
  ...
  public void addIngredient_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    NewIngredientRequest request =
        NewIngredientRequest.builder()
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.status_code", is(HttpStatus.OK.value())))
        .andExpect(
            jsonPath("$.message", is("Successfully added new ingredient with name 'Cucumber'")));
  }
  ...
  public void addIngredient_ShouldReturnConflict_WhenIngredientAlreadyExists() throws Exception {
    ...
    when(ingredientService.addIngredient(request))
        .thenThrow(new IngredientAlreadyExistsException(request.newIngredientName()));
  ...
  public void addIngredient_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(post("/api/v1/menu/add-ingredient").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void removeIngredient_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    Ingredient existingIngredient =
        Ingredient.builder().name("Jalapeno").ingredientType(IngredientType.VEGETABLE).build();

    RemovedIngredientRequest request = RemovedIngredientRequest.builder().name("Jalapeno").build();
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.status_code", is(HttpStatus.OK.value())))
        .andExpect(
            jsonPath("$.message", is("Successfully removed ingredient with name 'Jalapeno'")));
