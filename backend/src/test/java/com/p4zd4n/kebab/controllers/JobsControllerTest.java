...
public class JobsControllerTest {
  ...
  @InjectMocks private JobsController jobsController;
  ...
  public void getJobOffersForManager_ShouldReturnJobOffers_WhenCalled() throws Exception {
    ...
    List<JobOfferManagerResponse> jobOffers =
        Arrays.asList(
            JobOfferManagerResponse.builder().positionName("Cook").build(),
            JobOfferManagerResponse.builder().positionName("Cashier").build());

    when(jobOfferService.getJobOffersForManager()).thenReturn(jobOffers);

    mockMvc
        .perform(get("/api/v1/jobs/job-offers/manager"))
  ...
  public void getJobOffersForOtherUsers_ShouldReturnJobOffers_WhenCalled() throws Exception {
    ...
    List<JobOfferGeneralResponse> jobOffers =
        Arrays.asList(
            JobOfferGeneralResponse.builder().positionName("Cook").build(),
    ...
    mockMvc
        .perform(get("/api/v1/jobs/job-offers/general"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(2)))
        .andExpect(jsonPath("$[0].position_name", is("Cook")))
  ...
  public void addJobOffer_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    NewJobOfferRequest request =
        NewJobOfferRequest.builder()
            .positionName("Cook")
            .description("Description")
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.message", is("Successfully added new job offer with position name 'Cook'")));
  }

  @Test
  public void addJobOffer_ShouldReturnBadRequest_WhenInvalidRequest() throws Exception {

    NewJobOfferRequest request = NewJobOfferRequest.builder().positionName("Cook").build();
  ...
  public void addJobOffer_ShouldReturnConflict_WhenJobOfferExists() throws Exception {
    ...
    when(jobOfferService.addJobOffer(request))
        .thenThrow(new JobOfferAlreadyExistsException(request.positionName()));
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(new JobsController(jobOfferService, jobApplicationService))
  ...
  public void addJobOffer_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(post("/api/v1/jobs/add-job-offer").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void updateJobOffer_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    UpdatedJobOfferRequest request =
        UpdatedJobOfferRequest.builder()
            .positionName("Cook")
    ...
    when(jobOfferService.findJobOfferByPositionName(request.positionName()))
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.status_code", is(HttpStatus.OK.value())))
        .andExpect(
            jsonPath("$.message", is("Successfully updated job offer with position name 'Cook'")));
  ...
  @Test
  public void updateJobOffer_ShouldReturnConflict_WheJobOfferWithSameNameAlreadyExists()
      throws Exception {
    ...
    when(jobOfferService.findJobOfferByPositionName(request.positionName()))
        .thenReturn(existingJobOffer);
    when(jobOfferService.updateJobOffer(existingJobOffer, request))
        .thenThrow(new JobOfferAlreadyExistsException(request.updatedPositionName()));
  ...
  public void updateJobOffer_ShouldReturnBadRequest_WhenInvalidHeader() throws Exception {
    ...
    for (String header : invalidHeaders) {
      mockMvc
          .perform(put("/api/v1/jobs/update-job-offer").header("Accept-Language", header))
          .andExpect(status().isBadRequest());
    }
  }
  ...
  public void removeJobOffer_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    RemovedJobOfferRequest request = RemovedJobOfferRequest.builder().positionName("Cook").build();
    ...
    when(jobOfferService.findJobOfferByPositionName(existingJobOffer.getPositionName()))
        .thenReturn(existingJobOffer);
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.status_code", is(HttpStatus.OK.value())))
        .andExpect(
            jsonPath("$.message", is("Successfully removed beverage with position name 'Cook'")));
  ...
  public void addJobApplication_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    JobOfferApplicationResponse response =
        JobOfferApplicationResponse.builder()
            .statusCode(HttpStatus.OK.value())
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.message",
                is("Successfully added new job offer application on position 'Cook'")));
  }
  ...
  public void removeApplication_ShouldReturnOk_WhenValidRequest() throws Exception {
    ...
    RemovedApplicationRequest request =
        RemovedApplicationRequest.builder().positionName("Cook").applicationId(1L).build();

    RemovedApplicationResponse response =
        RemovedApplicationResponse.builder()
            .statusCode(HttpStatus.OK.value())
            .message(
    ...
    when(jobOfferService.findJobOfferByPositionName(request.positionName()))
        .thenReturn(existingJobOffer);
    when(jobApplicationService.removeJobApplication(request.applicationId(), existingJobOffer))
        .thenReturn(response);
    ...
    verify(jobApplicationService, times(1))
        .removeJobApplication(request.applicationId(), existingJobOffer);
  }

  @Test
  public void addCv_ShouldReturnOk_WhenCvIsUploaded() throws Exception {
    Long applicationId = 1L;
    MockMultipartFile cvFile =
        new MockMultipartFile(
            "cv", "cv.pdf", MediaType.APPLICATION_PDF_VALUE, "Test CV content".getBytes());

    NewCvResponse response =
        NewCvResponse.builder()
            .statusCode(HttpStatus.OK.value())
            .message("Successfully added cv with name 'cv.pdf' to job application with id '1'!")
            .build();

    when(jobApplicationService.addCv(any(MultipartFile.class), eq(applicationId)))
        .thenReturn(response);

    mockMvc
        .perform(
            multipart("/api/v1/jobs/add-cv")
                .file(cvFile)
        ...
        .andExpect(
            jsonPath(
                "$.message",
                is("Successfully added cv with name 'cv.pdf' to job application with id '1'!")));
  ...
  public void downloadCv_ShouldReturnCvFile_WhenIdIsValid() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/jobs/download-cv/{id}", cvId))
        .andExpect(status().isOk())
        .andExpect(
            header().string(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"cv.pdf\""))
