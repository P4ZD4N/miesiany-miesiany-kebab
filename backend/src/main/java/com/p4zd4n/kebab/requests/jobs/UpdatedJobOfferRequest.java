...
public record UpdatedJobOfferRequest(
    @JsonProperty("position_name") @Size(min = 1, message = "{name.greaterThanZero}")
        String positionName,
    @JsonProperty("updated_position_name") @Size(min = 1, message = "{name.greaterThanZero}")
        String updatedPositionName,
    @JsonProperty("updated_description") @Size(min = 1, message = "{description.greaterThanZero}")
