...
public record JobOfferApplicationRequest(
    @JsonProperty("position_name") @Size(min = 1, message = "{name.greaterThanZero}")
        String positionName,
    @JsonProperty("applicant_first_name") @Size(min = 1, message = "{firstName.greaterThanZero}")
        String applicantFirstName,
    @JsonProperty("applicant_last_name") @Size(min = 1, message = "{last.greaterThanZero}")
