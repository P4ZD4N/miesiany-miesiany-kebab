...
public class NewsletterService {
  ...
  public List<NewsletterSubscriberResponse> getSubscribers() {
    ...
    List<NewsletterSubscriberResponse> response =
        subscribers.stream().map(this::mapToResponse).collect(Collectors.toList());
  ...
  public NewNewsletterSubscriberResponse addNewsletterSubscriber(
      NewNewsletterSubscriberRequest request) throws MessagingException {
    ...
    else verificationMailUtil.sendPl(request.email(), otp);
  ...
  public VerifyNewsletterSubscriptionResponse verifySubscription(
      VerifyNewsletterSubscriptionRequest request) throws MessagingException {
    ...
    boolean otpExpired =
        !(Duration.between(subscriber.getOtpGeneratedTime(), LocalDateTime.now()).getSeconds()
            < OtpUtil.OTP_EXPIRATION_SECONDS);
    ...
    else welcomeMailUtil.sendPl(subscriber);

    return VerifyNewsletterSubscriptionResponse.builder()
        .statusCode(HttpStatus.OK.value())
        .message(
            "Successfully verified newsletter subscriber with '" + request.email() + "' email!")
        .build();
  }

  public RegenerateOtpResponse regenerateOtp(RegenerateOtpRequest request)
    ...
    if (Duration.between(subscriber.getOtpGeneratedTime(), LocalDateTime.now()).getSeconds()
        < OtpUtil.OTP_REGENERATION_TIME_SECONDS) {
    ...
    else verificationMailUtil.sendPl(request.email(), newOtp);
  ...
  public UnsubscribeResponse unsubscribe(UnsubscribeRequest request) throws MessagingException {
    ...
    else goodbyeMailUtil.sendPl(subscriber);
