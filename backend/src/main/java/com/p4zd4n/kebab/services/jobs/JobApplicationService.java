...
public class JobApplicationService {
  ...
  public JobOfferApplicationResponse addJobOfferApplication(JobOfferApplicationRequest request) {
    ...
    JobApplication newJobApplication =
        JobApplication.builder()
            ...
            .build();
    newJobApplication.setJobOffer(
        jobOfferRepository
            .findByPositionName(request.positionName())
            .orElseThrow(() -> new JobOfferNotFoundException(request.positionName())));
    ...
    JobOfferApplicationResponse response =
        JobOfferApplicationResponse.builder()
            ...
            .message(
                "Successfully added new job offer application on position '"
                    + request.positionName()
                    + "'")
            .build();

    log.info(
        "Successfully added new job offer application on position '{}'", request.positionName());
  ...
  public NewCvResponse addCv(MultipartFile cv, Long applicationId) throws IOException {
    ...
    Cv newCv = Cv.builder().fileName(cv.getOriginalFilename()).data(cv.getBytes()).build();
    ...
    NewCvResponse response =
        NewCvResponse.builder()
            .statusCode(HttpStatus.OK.value())
            .message(
                "Successfully added cv with name '"
                    + cv.getOriginalFilename()
                    + "' to job application with id '"
                    + applicationId
                    + "'!")
            .build();
    ...
    log.info(
        "Successfully added cv with name: '{}' to job application with id: '{}'",
        cv.getOriginalFilename(),
        applicationId);
  ...
  public ResponseEntity<byte[]> getCv(Long id) {
    ...
    Cv cv = cvRepository.findById(id).orElseThrow(() -> new CvNotFoundException(id));

    log.info("Successfully retrieved CV with id '{}'", id);

    return ResponseEntity.ok()
        .header(
            HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + cv.getFileName() + "\"")
