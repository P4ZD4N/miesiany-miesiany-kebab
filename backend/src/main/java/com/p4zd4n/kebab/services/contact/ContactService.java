...
public class ContactService {
  ...
  public List<ContactResponse> getContacts() {
    ...
    List<ContactResponse> response =
        contacts.stream().map(this::mapToResponse).collect(Collectors.toList());
  ...
  public UpdatedContactResponse updateContact(Contact contact, UpdatedContactRequest request) {
    ...
    UpdatedContactResponse response =
    ...
    if (contact.getContactType().equals(ContactType.TELEPHONE)
        && (request.newValue().length() != 9 || !request.newValue().matches("\\d{9}"))) {
