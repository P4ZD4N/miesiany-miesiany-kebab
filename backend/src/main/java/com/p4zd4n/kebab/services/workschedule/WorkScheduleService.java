...
public class WorkScheduleService {
  ...
  public List<WorkScheduleEntryResponse> getWorkScheduleEntries() {
    ...
    List<WorkScheduleEntryResponse> response =
        workScheduleEntries.stream().map(this::mapToResponse).collect(Collectors.toList());
  ...
  public NewWorkScheduleEntryResponse addWorkScheduleEntry(NewWorkScheduleEntryRequest request) {
    ...
    Employee employee =
        employeeRepository
            .findByEmail(request.employeeEmail())
            .orElseThrow(() -> new EmployeeNotFoundException(request.employeeEmail()));

    List<WorkScheduleEntry> existingEntries =
        workScheduleEntryRepository.findByEmployeeEmailAndDate(
            request.employeeEmail(), request.date());
    ...
    existingEntries.forEach(
        entry -> {
          ...
          if (newStartTime.isBefore(existingEndTime) && newEndTime.isAfter(existingStartTime)) {
            throw new WorkScheduleTimeOverlapException(
                request.employeeEmail(), request.date(), newStartTime, newEndTime);
          }

          if (newStartTime.equals(existingStartTime) && newEndTime.equals(existingEndTime)) {
            throw new WorkScheduleEntryAlreadyExistsException(
                request.employeeEmail(), request.date(), newStartTime, newEndTime);
          }
        });

    WorkScheduleEntry newWorkScheduleEntry =
    ...
    return NewWorkScheduleEntryResponse.builder()
        ...
        .message(
            "Successfully added new work schedule entry for employee with email '"
  ...
  public RemovedWorkScheduleEntryResponse removeWorkScheduleEntry(
      WorkScheduleEntry workScheduleEntry) {
    ...
    RemovedWorkScheduleEntryResponse response =
        RemovedWorkScheduleEntryResponse.builder()
            .statusCode(HttpStatus.OK.value())
            .message(
                "Successfully removed work schedule entry with id '"
                    + workScheduleEntry.getId()
                    + "'")
            .build();
  ...
  public byte[] generateWorkSchedulePDF(LocalDate startDate, LocalDate endDate, String language)
  ...
  private void addDocumentTable(Document document, LocalDate startDate, LocalDate endDate)
      throws DocumentException {
    Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
    String authenticatedEmail = authentication.getName();

    Font tableFont = new Font(Font.FontFamily.HELVETICA, 8);
    List<WorkScheduleEntry> filteredWorkScheduleEntries =
        workScheduleEntryRepository.findAll().stream()
            .filter(
                entry -> !entry.getDate().isBefore(startDate) && !entry.getDate().isAfter(endDate))
            .toList();
    List<LocalDate> daysInMonth = startDate.datesUntil(endDate.plusDays(1)).toList();
    Map<String, Employee> employeesByEmail =
        employeeRepository.findAll().stream().collect(Collectors.toMap(Employee::getEmail, e -> e));
    Map<Employee, List<WorkScheduleEntry>> entriesByEmployee =
        filteredWorkScheduleEntries.stream()
            ...
            .collect(Collectors.groupingBy(e -> employeesByEmail.get(e.getEmployeeEmail())));
    ...
    entriesByEmployee.forEach(
        (employee, workSchedule) -> {
          ...
          daysInMonth.forEach(
              date -> {
                List<WorkScheduleEntry> entriesForDay =
                    workSchedule.stream().filter(entry -> entry.getDate().equals(date)).toList();

                if (entriesForDay.isEmpty()) {
                  table.addCell(
                      createCenteredCell(
                          "", tableFont, isCurrentUser ? BaseColor.RED : BaseColor.WHITE));
                } else {
