...
export class ContactService {
  ...
  getContacts(): Observable<ContactResponse[]> {
    return this.http.get<ContactResponse[]>(`${this.apiUrl}/contacts`, {
  ...
  ): Observable<UpdatedContactResponse> {
    const headers = new HttpHeaders({
      'Accept-Language': this.langService.currentLang,
    });

    return this.http
      .put<UpdatedContactResponse>(`${this.apiUrl}/update-contact`, contact, {
        headers,
        withCredentials: true,
      })
      .pipe(
